...
app.get('/', (req, res) => res.send('Hello World! This is the backend homepage'));


app.listen(port, () => console.log(`App is listening on port ${port}!`));

/** DEBUG */
//:id is the user input
// id becomes the var name and the value is what the user input as :id
app.get('/products/:id', function (req, res, next) {
  res.json({msg: 'This is CORS-enabled for all origins!'});
})
// all inputs are available in json object called req.params
app.get('/reqparams/keyword=:keyword/zip=:zip', function (req, res) {
  res.send(req.params);
})

// DEBUGGING PURPOSES: http request via node request module 
// attach user input variables (from backend call) to base url of external api call
app.get('/ipapi/:filetype', function(req, res, next) {
  var apiURL = 'http://ip-api.com/';
  request({url: apiURL + req.params.filetype }).pipe(res);
});
...
function createEbayFindingOptions(myparams) {
  ...
  if (myparams.miles != "none") {
    baseURL += `&buyerPostalCode=${myparams.zipcode}`;
